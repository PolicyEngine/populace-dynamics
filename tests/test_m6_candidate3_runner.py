"""Guard and orchestration tests for the hash-bound M6 candidate-3 runner."""
...
from populace_dynamics.harness import m6_candidate3_runner as runner
...
def _candidate_specs() -> dict:
    return {
        "family_transitions": (
            family_candidates.M6_CANDIDATE_3.canonical_dict()
        ),
        "engine_operations": engine_candidates.CANDIDATE_3.canonical_dict(),
    }
...
def _matching_bindings(**changes) -> M6Candidate3Bindings:
    ...
    bindings = M6Candidate3Bindings(
        ...
        candidate_spec_ids={
            ...
            "engine_operations": "m6_candidate3_engine_v1",
        },
...
def _preparation(tmp_path: Path) -> M6Candidate3Preparation:
...
def _replace_surface(
    bindings: M6Candidate3Bindings, surface: str
) -> M6Candidate3Bindings:
    bad = "f" * 64
...
def test_dirty_tree_aborts_before_any_data_read(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="dirty worktree"):
        runner.guard_registered_m6_candidate3_run(
            registration_id="9999999999",
            root=tmp_path,
        )
...
@pytest.mark.parametrize(
    "forbidden",
    [runner.CANDIDATE1_OUTPUT, runner.CANDIDATE2_OUTPUT],
)
def test_prior_candidate_output_paths_are_hard_refused(
    forbidden, monkeypatch, tmp_path
):
    assert runner.CANDIDATE1_OUTPUT == Path("runs/gate_m6_candidate1_v1.json")
    assert runner.CANDIDATE2_OUTPUT == Path("runs/gate_m6_candidate2_v1.json")
    assert runner.DEFAULT_OUTPUT == Path("runs/gate_m6_candidate3_v1.json")
    monkeypatch.setattr(
        runner,
...
@pytest.mark.parametrize(
    "registration_id",
    sorted(runner.CONSUMED_CANDIDATE_REGISTRATION_IDS),
)
@pytest.mark.parametrize("as_url", [False, True])
def test_consumed_prior_registration_ids_are_hard_refused(
    registration_id,
    as_url,
):
    registration = (
        (
            ...
            f"#issuecomment-{registration_id}"
        )
        if as_url
        else registration_id
    )
    with pytest.raises(ValueError, match="fresh registration"):
        runner.validate_candidate3_registration_id(registration)


def test_every_non_candidate3_output_path_is_refused(monkeypatch, tmp_path):
    monkeypatch.setattr(
        runner,
        "capture_source_identity",
        lambda _root: (_ for _ in ()).throw(
...
def test_preregistered_binding_mismatch_aborts_before_data_read(
    ...
    monkeypatch.setattr(
        runner, "assert_candidate3_identity_is_frozen", lambda _: None
    )
    monkeypatch.setattr(
        runner,
        "collect_candidate3_bindings",
...
def test_candidate3_identity_comes_from_registry_and_is_not_candidate16():
    identity = runner.resolve_candidate3_identity()
    ...
    assert identity.family_spec is family_candidates.M6_CANDIDATE_3
    ...
    runner.assert_candidate3_identity_is_frozen(identity)


def test_candidate3_identity_rejects_candidate16(monkeypatch):
    for name in ("M6_CANDIDATE_2", "M6_CANDIDATE_3"):
        monkeypatch.setattr(
            family_candidates,
            name,
    ...
    with pytest.raises(RuntimeError, match="forbidden candidate 16"):
        runner.resolve_candidate3_identity()


def test_candidate3_identity_rejects_non_first_marriage_sibling_drift(
    monkeypatch,
):
    spec = family_candidates.M6_CANDIDATE_3
    components = tuple(
        (
            replace(
    ...
    drifted = replace(spec, components=components)
    for name in ("M6_CANDIDATE_2", "M6_CANDIDATE_3"):
        monkeypatch.setattr(family_candidates, name, drifted)
...
def test_registry_values_are_semantically_bound_to_selection_ledgers():
    ...
    rhostar = runner._canonical_json_bytes(
        {
            "schema": "m6_rhostar_train_only_selection.findings.v1",
            "status": "SELECTION_COMPLETE",
            "selector": {
                "disposition": "LOCK_ADDENDUM_ELIGIBLE",
                "selected_rho": -0.60,
            },
            "protocol": {"fixed_q": 0.55},
            "cross_pins": {
                "candidate2_engine_spec_sha256": (
                    engine_candidates.CANDIDATE_2.sha256
                ),
                "qstar_ledger_sha256": hashlib.sha256(qstar).hexdigest(),
                "qstar_selected_q": 0.55,
            },
            "fences": {
    ...
    with pytest.raises(RuntimeError, match="engine rho differs"):
        runner._assert_selection_semantics(
            frozen,
            {
                **evidence,
                "rhostar_rho_grid": runner._canonical_json_bytes(
                    {
                        **json.loads(rhostar),
                        "selector": {
                            "disposition": "LOCK_ADDENDUM_ELIGIBLE",
    ...
    with pytest.raises(RuntimeError, match="cross-pins drifted"):
        rhostar_document = json.loads(rhostar)
        rhostar_document["cross_pins"]["qstar_ledger_sha256"] = "0" * 64
        runner._assert_selection_semantics(
            frozen,
            {
                **evidence,
...
def test_prefreeze_identity_aborts_before_binding_collection(
    ...
    monkeypatch.setattr(runner, "collect_candidate3_bindings", collect)
    monkeypatch.setattr(
        runner,
        "capture_source_identity",
        lambda _root: M6SourceIdentity("a" * 40),
    ...
    # Keep artifact-sensitive guard coverage isolated from repository runs.
    (tmp_path / runner.DEFAULT_OUTPUT.parent).mkdir()
...
def test_live_preregistered_file_and_spec_hashes_are_exact():
    ...
    identity = runner.resolve_candidate3_identity()
    gate_document = runner._live_gate_document(ROOT, head)
    assert runner._gate_design_commit(gate_document) == (
        runner.PREREGISTERED_VALUES["design_commit"]
    )
    assert (
...
def test_gate_environment_requires_exact_versions_threads_and_editables(
    monkeypatch,
):
    versions = dict(runner.EXPECTED_RUNTIME_VERSIONS)
    package_versions = {
        "pandas": versions["pandas"],
        "scikit-learn": versions["scikit_learn"],
        "scipy": versions["scipy"],
        "quantile-forest": versions["quantile_forest"],
        "populace-fit": versions["populace_fit"],
        "populace-frame": versions["populace_frame"],
    ...
    def editable(package, **kwargs):
        editable_calls.append((package, kwargs))
        return {
            "version": package_versions[package],
            "editable_source": str(kwargs["source_directory"]),
            "repository_root": str(runner.PINNED_POPULACE_ROOT),
            "repository_head": runner.EXPECTED_POPULACE_HEAD,
            "source_tree_sha1": kwargs["expected_tree"],
            "tracked_source_clean": True,
        }

    monkeypatch.setattr(runner, "_editable_source_snapshot", editable)
    for key, value in runner.EXPECTED_THREAD_ENVIRONMENT.items():
        monkeypatch.setenv(key, value)
    monkeypatch.setenv("OMP_WAIT_POLICY", "ACTIVE")
    monkeypatch.setenv("UNRELATED_CANDIDATE3_ENV", "allowed")

    snapshot = runner._frozen_gate_environment()

    assert snapshot["runtime"] == versions
    assert snapshot["thread_environment"] == dict(
        runner.EXPECTED_THREAD_ENVIRONMENT
    )
    assert snapshot["omp_wait_policy"] == "ACTIVE"
    assert [call[0] for call in editable_calls] == [
        "populace-fit",
        "populace-frame",
    ]
    assert editable_calls[0][1]["source_directory"] == (
        runner.PINNED_POPULACE_ROOT / "packages" / "populace-fit"
    )
    assert (
        editable_calls[0][1]["expected_tree"]
        == runner.EXPECTED_POPULACE_FIT_TREE
    )
    assert (
        editable_calls[1][1]["expected_tree"]
        == runner.EXPECTED_POPULACE_FRAME_TREE
    )

    monkeypatch.setenv("OMP_NUM_THREADS", "7")
    with pytest.raises(RuntimeError, match="exact eight-variable"):
        runner._frozen_gate_environment()
    monkeypatch.setenv("OMP_NUM_THREADS", "8")
    monkeypatch.setenv("OMP_WAIT_POLICY", "PASSIVE")
    with pytest.raises(RuntimeError, match="OMP_WAIT_POLICY"):
        runner._frozen_gate_environment()


def test_gate_environment_pin_constants_are_the_ratified_lane():
    assert dict(runner.EXPECTED_RUNTIME_VERSIONS) == {
        "python": "3.14.4",
        "numpy": "2.5.1",
        "pandas": "3.0.3",
        "scikit_learn": "1.8.0",
    ...
    assert runner.EXPECTED_POPULACE_HEAD == (
        "ee8f7fc139271de5d4e448549c35e8c5eb992534"
    )
    assert runner.EXPECTED_POPULACE_FIT_TREE == (
        "5c866378fdf5906b7a61da9977b8d028d1d36e9f"
    )
    assert runner.EXPECTED_POPULACE_FRAME_TREE == (
...
def test_binding_collection_records_design_hashes_from_captured_commit(
    ...
    source = M6SourceIdentity("a" * 40)
    bindings, _, _ = runner.collect_candidate3_bindings(
        tmp_path,
        source=source,
        identity=runner.resolve_candidate3_identity(),
    )
...
def test_must_not_regress_is_per_cell_four_of_five_without_common_seed():
    first_target = "earn_dlog_sd.older"
    second_target = "remarriage.18-64"
    gate = _fake_gate(
        seed_scores={
            0: {
                first_target: runner.MUST_NOT_REGRESS_TOLERANCES[first_target]
            },
            3: {
                second_target: (
                    runner.MUST_NOT_REGRESS_TOLERANCES[second_target] + 0.001
                )
            },
            4: {
                first_target: (
                    runner.MUST_NOT_REGRESS_TOLERANCES[first_target] + 0.001
                )
            },
        }
    )
    ...
    assert result["aggregation_unit"] == "cell"
    assert result["same_seed_conjunction_required"] is False
    assert result["per_seed"][0]["cells"][first_target]["pass"] is True
    assert result["per_cell"][first_target]["n_seeds_pass"] == 4
    assert result["per_cell"][second_target]["n_seeds_pass"] == 4
    assert (
        sum(
            record["all_constrained_cells_pass"]
            for record in result["per_seed"]
        )
        == 3
    )
    assert result["pass"] is True
    assert set(result["candidate2_five_of_five_cells"]) == set(
        runner.CANDIDATE2_FIVE_OF_FIVE_CELLS
    )
    assert result["candidate2_four_of_five_cells"] == ["remarriage.18-64"]
    assert result["target_surface_exclusions"] == [
...
def test_must_not_regress_fails_when_only_three_seeds_clear():
    ...
    assert result["per_cell"][target]["n_seeds_pass"] == 3
    assert result["per_cell"][target]["pass"] is False
...
def test_gate_pass_regression_fail_is_never_an_accepted_pass():
    ...
    acceptance = runner._candidate3_acceptance(
...
def test_candidate3_acceptance_statuses(
    gate_valid, gate_pass, regression_valid, expected
):
    regression = runner._must_not_regress_artifact(
...
def test_must_not_regress_validity_uses_only_its_nine_constrained_cells():
...
def test_first_marriage_decomposition_fails_closed_on_invalid_paired_arm():
    ...
    verdict = runner._candidate3_run_verdict(
...
def test_sidecar_is_canonical_and_sha_is_bound_in_main_artifact(tmp_path):
    ...
    artifact = runner.assemble_m6_candidate3_artifact(
        registration_id="9999999999",
    ...
    runner.write_new_candidate3_artifact(
    ...
    assert set(artifact["candidate3_acceptance"]) == {
...
@pytest.mark.parametrize(
    "name",
    ["gate_m6_candidate1_v1.json", "gate_m6_candidate2_v1.json"],
)
def test_pair_writer_hard_refuses_prior_candidate_paths(name, tmp_path):
    sidecar = _sidecar()
    artifact = {
        "integrity": {
            "environment_sidecar": {
    ...
    with pytest.raises(ValueError, match="hard-refuses"):
        runner.write_new_candidate3_artifact(path, artifact, sidecar)
    assert not path.exists()


def test_default_refit_binds_both_candidate3_specs(monkeypatch):
    sentinel = object()
    observed = {}
    ...
    inputs = object()
    assert runner._fit_candidate3(inputs) is sentinel
    identity = runner.resolve_candidate3_identity()
    assert observed == {
        "inputs": inputs,
...
def test_postrepair_incumbent_comparator_swaps_only_first_marriage(
    ...
    candidate_fitted = Fitted(
        first_marriage=object(),
        **carried,
        implementation_ids={"first_marriage": "candidate3"},
    )
    ...
    candidate_family = refit_module.RegistryRefit(
        candidate_id="m6_candidate3_registry_v1",
        spec_sha256="a" * 64,
...
def test_fit_digest_accepts_populated_candidate3_audit_core():
    core = _refit_registered_family_core(family_candidates.M6_CANDIDATE_3)
...
def test_every_live_registered_family_core_is_digestable():
    specs = (
        family_candidates.CANDIDATE_16,
        family_candidates.M6_CANDIDATE_3,
    )
...
def test_candidate3_default_preflight_threads_registered_family_spec(
    monkeypatch,
):
    identity = runner.resolve_candidate3_identity()
    calls = []
...
def _transport_universe_operations(
    ...
    def fit_preflight(bundle):
        ...
        return runner._preflight_candidate3_first_marriage(bundle)
...
def _install_transport_universe_test_seams(
    ...
    registered_identity = runner.resolve_candidate3_identity()
    monkeypatch.setattr(
        runner,
...
def test_transport_universe_end_to_end_scores_and_writes_artifact(
    ...
    artifact = runner.execute_registered_m6_candidate3_run(
        runner.M6Candidate3InputPlan(
...
def test_transport_universe_pre_fix_join_reproduces_designed_abort(
    ...
    with pytest.raises(runner.M6Candidate3DesignedAbort) as caught:
        runner.execute_registered_m6_candidate3_run(
            runner.M6Candidate3InputPlan(
                fit_inputs=fixture.fit_inputs,
                load_full_inputs=load_full_inputs,
            ),
...
def test_candidate3_runner_orders_phases_and_commits_last(
    monkeypatch, tmp_path
):
    preparation = _preparation(tmp_path)
    events = []
    ...
    assert artifact["candidate"]["number"] == 3
    assert artifact["run"]["name"] == "gate_m6_candidate3"
