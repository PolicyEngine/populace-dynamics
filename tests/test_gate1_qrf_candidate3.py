"""Tests for the gate-1 candidate-3 persistence-aware decomposition run.

Mirrors the candidate-2 run's tests (``test_gate1_qrf_candidate3`` is a
one-for-one analogue of ``test_gate1_qrf_candidate2``):
...
* :func:`test_seed0_reproduces_committed_artifact` (skipped when the PSID
  ...
  reruns seed 0 through the candidate-3 stages (stage 0 imported from
  candidate 2; the amended stages 1a-1c local; stages 2-3 imported) and
  pins the committed artifact's seed-0 stage-1 fit, geometry, and battery
  values to float precision — the run is reproducible from the seeds
  alone.
* The always-runnable consistency tests touch only the committed artifact
  ...
  seed-conjunction table, and the persistence-aware stage-1 diagnostics
...
ARTIFACT = ROOT / "runs" / "gate1_qrf_latent_perm_v2.json"
...
def test_seed0_reproduces_committed_artifact():
    ...
    import run_gate1_candidate3 as runner
    ...
    # Stage 1a-1c (amended): the persistence-aware decomposition.
    ...
    # Stage 2 (imported): fit P(perm | anchor), draw holdout perms.
    ...
    # Stage 3 (imported): perm-conditioned backward chain.
...
def test_artifact_present_and_locked():
    artifact = _artifact()
    assert artifact["schema_version"] == "gate1_qrf_latent_perm.v2"
...
def test_spec_registration_recorded():
    """The frozen candidate-3 spec URL (and base) are in the artifact."""
    artifact = _artifact()
    assert artifact["spec_registration"] == SPEC_URL
    assert artifact["base_registration"] == BASE_URL
    assert artifact["amends"] == (
        "candidate 2 stage 1 only (all other stages identical)"
    )
...
def test_stage1_fit_is_reported_not_gated_and_consistent():
    ...
    Every seed carries the amended stage-1 estimates (gamma_k, chosen
    rho, the three component variances, the implied perm share); the
    top-level context block mirrors them and records the memo's 0.467
    back-out and candidate 2's 0.52-0.55 drawn range. These are context
    only — they never enter the geometry or battery pass/fail, and the
    verdict rule names only those two families.
    """
    artifact = _artifact()
    ctx = artifact["stage1_fit_context"]
    ...
    # battery only; the stage-1 share is nowhere in it.
