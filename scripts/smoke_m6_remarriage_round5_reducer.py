#!/usr/bin/env python3
"""Exercise four remarriage reducer revisions on one synthetic cube.
...
import reduce_m6_remarriage_round5 as reducer
...
ROUND5_REDUCER = ROOT / "scripts/reduce_m6_remarriage_round5.py"
...
ZERO_EXPOSURE_GROUPS = {
    "2006": (11,),
    "2008": (11,),
    "2010": (2, 11, 13),
}
MIXED_NULL_BOUNDARY = "2008"
MIXED_NULL_GROUP_INDEX = 10
MIXED_NULL_SEEDS = tuple(block[0] for block in reducer.BLOCKS)
NULL_RATE_PAYLOAD = {
    "risk_rows": 0,
    "event_rows": 0,
    "exposure": 0.0,
...
def _direct(truth: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_rate = expected_numerator / exposure if exposure else None
    weighted_deviance = (
        weighted_deviance_numerator / exposure if exposure else None
    )
    return {
        ...
        "weighted_bernoulli_deviance": weighted_deviance,
    }
...
def _projected_groups(
    truth_groups: list[dict[str, Any]], *, boundary: str, seed: int
) -> list[dict[str, Any]]:
    groups = []
    ...
    if boundary == MIXED_NULL_BOUNDARY and seed in MIXED_NULL_SEEDS:
        truth = truth_groups[MIXED_NULL_GROUP_INDEX]
        truth_payload = {
            key: value
            for key, value in truth.items()
            if key not in GROUP_IDENTITY_FIELDS
        }
        groups[MIXED_NULL_GROUP_INDEX] = {
            **{key: truth[key] for key in GROUP_IDENTITY_FIELDS},
            **copy.deepcopy(NULL_RATE_PAYLOAD),
            **reducer._ratio_fields(NULL_RATE_PAYLOAD, truth_payload),
        }
    return groups
...
def _truth(locked: dict[str, Any], *, boundary: str) -> dict[str, Any]:
    group_truth = {
        "risk_rows": 1,
        "event_rows": 1,
        "exposure": 2.0,
    ...
    for index in ZERO_EXPOSURE_GROUPS[boundary]:
        groups[index].update(copy.deepcopy(NULL_RATE_PAYLOAD))
    return {
...
def _seed_row(
    *,
    ...
    return {
        ...
        "publication_groups": _projected_groups(
            truth["groups"], boundary=boundary, seed=seed
        ),
        "carrier_checks": {
...
def _boundary_record(
    *, boundary: str, fit: dict[str, Any], config: dict[str, Any]
    ...
    truth = _truth(locked, boundary=boundary)
...
def _with_aggregate_group_rate(
    raw: bytes, *, aggregate_name: str, group_index: int, rate: float | None
) -> bytes:
    ledger = json.loads(raw)
    law_record = ledger["boundaries"]["2006"]["laws"]["R0"]
    aggregate = (
        law_record["mean"]
        if aggregate_name == "mean"
        else law_record["blocks"][aggregate_name]
    )
    aggregate["groups"][group_index]["rate"] = rate
    return json.dumps(
        ledger,
        allow_nan=False,
        separators=(",", ":"),
...
def _aggregate_null_rate_census(raw: bytes) -> tuple[int, int, int]:
    ledger = json.loads(raw)
    checked = 0
    null_rates = 0
    zero_exposure = 0
    ...
    for boundary in reducer.BOUNDARIES:
        for law in reducer.LAWS:
            law_record = ledger["boundaries"][boundary]["laws"][law]
            rows = law_record["per_seed"]
            aggregates = (
                (law_record["mean"], reducer.SEEDS),
                (law_record["blocks"]["block_1"], reducer.BLOCKS[0]),
                (law_record["blocks"]["block_2"], reducer.BLOCKS[1]),
            )
            for aggregate, seeds in aggregates:
                contributors = [row for row in rows if row["seed"] in seeds]
                for index, group in enumerate(aggregate["groups"]):
                    checked += 1
                    stored_null = group["rate"] is None
                    any_seed_null = any(
                        row["publication_groups"][index]["rate"] is None
                        for row in contributors
                    )
                    if stored_null != any_seed_null:
                        raise ValueError(
                            "synthetic aggregate rate violated the null "
                            "biconditional"
                        )
                    if not stored_null:
                        continue
                    null_rates += 1
                    if (
                        group["rate_ratio"] is not None
                        or group["log_rate_ratio"] is not None
                    ):
                        raise ValueError(
                            "synthetic null rate did not null-propagate ratios"
                        )
                    if float(group["exposure"]) == 0.0:
                        zero_exposure += 1
                    else:
                        positive_exposure += 1
    census = (null_rates, zero_exposure, positive_exposure)
    if checked != 810 or census != (90, 75, 15):
        raise ValueError(
            f"synthetic aggregate null-rate census drifted: "
            f"checked={checked}, census={census}"
        )
    return census
...
def main() -> int:
    raw = _fixture()
    null_rates, zero_exposure, positive_exposure = _aggregate_null_rate_census(
        raw
    )
    ...
    if (
        ...
        raise ValueError(
            "round-5 reducer did not reject an unknown calibration key"
        )

    biconditional_probes = (
        ("mean", 0, None),
        ("block_1", 11, 0.0),
    )
    for aggregate_name, group_index, rate in biconditional_probes:
        mismatch = _run_reducer(
            ROUND5_REDUCER,
            _with_aggregate_group_rate(
                raw,
        ...
        mismatch_error = mismatch.stderr.decode().strip().splitlines()[-1]
        aggregate_where = (
            "mean" if aggregate_name == "mean" else f"blocks.{aggregate_name}"
        )
        expected_mismatch_error = (
            f"ValueError: boundaries.2006.laws.R0.{aggregate_where}.groups"
            f"[{group_index}].rate must be null iff at least one "
            "contributing per-seed rate is null"
        )
        if (
            mismatch.returncode != 1
            or mismatch_error != expected_mismatch_error
        ):
            raise ValueError(
                "round-5 reducer did not enforce both directions of the "
                "aggregate null-rate biconditional"
            )

    round4 = _run_reducer(ROUND4_REDUCER, raw)
    round4_error = round4.stderr.decode().strip().splitlines()[-1]
    expected_round4_error = (
        "ValueError: boundaries.2006.laws.R0.mean.groups[11].rate "
        "must be a JSON number"
    )
    if round4.returncode != 1 or round4_error != expected_round4_error:
        raise ValueError(
            "round-4 reducer did not fail at the aggregate null-rate contract"
        )
    print(
        "ROUND4_REDUCER_SYNTHETIC_SMOKE=EXPECTED_FAIL "
        f'exit=1 error="{round4_error}"'
    )

    round5 = _run_reducer(ROUND5_REDUCER, raw)
    if round5.returncode != 0:
        raise ValueError(round5.stderr.decode())
    findings = json.loads(round5.stdout)
    removed = findings["reducer"]["removed_publication_groups_arrays"]
    if findings["selection"]["selected_law"] != "R0" or removed != 600:
        raise ValueError("round-5 reducer did not traverse the full cube")
    print(
        "ROUND5_REDUCER_SYNTHETIC_SMOKE=PASS exit=0 "
        f"selected_law=R0 removed_arrays={removed} "
        f"null_rate_aggregates={null_rates} "
        f"zero_exposure={zero_exposure} "
        f"positive_exposure={positive_exposure} "
        f"biconditional_rejections={len(biconditional_probes)} "
        "strict_serialization=PASS aggregate_assert_same=PASS"
    )
    return 0
