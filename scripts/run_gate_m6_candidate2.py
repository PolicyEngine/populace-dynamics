#!/usr/bin/env python3
"""Run the hash-bound registered M6 candidate-2 gate exactly once.

The command has no candidate selector: candidate number 2 and both model specs
...
def _input_factory(specification: str) -> Callable[[], M6Candidate2InputPlan]:
...
def parser() -> argparse.ArgumentParser:
    command = argparse.ArgumentParser(
        description=(
            "ONE-SHOT hash-bound M6 candidate-2 run. Requires a fresh #42 "
    ...
    command.add_argument(
        ...
        help=(
            ...
            "M6Candidate2InputPlan whose fit-only "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    preparation = guard_registered_m6_candidate2_run(
    ...
    if not isinstance(input_plan, M6Candidate2InputPlan):
        raise TypeError(
            "input factory must return M6Candidate2InputPlan; eager "
            "M6HarnessInputs are prohibited"
        )
    try:
        artifact = execute_registered_m6_candidate2_run(
            input_plan,
            registration_id=args.registration_id,
            output=args.out,
            root=ROOT,
    ...
    except M6Candidate2DesignedAbort as error:
        print(
            json.dumps(
                error.report,
                sort_keys=True,
    ...
    print(
        f"M6 candidate 2 {verdict}: valid={artifact['verdict']['valid']} "
        f"out={args.out}. Certifies nothing about mortality drift. "
        "Gated earnings use the registered candidate-2 rank-refresh law."
    )
    return 0
