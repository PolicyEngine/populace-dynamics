#!/usr/bin/env python3
"""Strictly validate and reduce the frozen round-2 selector stdout.

Usage::

    python scripts/reduce_m6_remarriage_round2.py \
      < docs/analysis/m6_remarriage_round2_selection_full.json \
      > docs/analysis/m6_remarriage_round2_selection_results.json
...
CONFIG_FILENAME = "m6_remarriage_round2_selector_config.json"
CONFIG_SCHEMA = "m6.remarriage.round2.selector_config.v1"
RAW_SCHEMA = "m6.remarriage.learning_plan.round2.selection.full.v1"
FINDINGS_SCHEMA = "m6.remarriage.learning_plan.round2.selection.findings.v1"
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    expected_output = {
        "selector_script": "scripts/select_m6_remarriage_round2.py",
        "config_file": f"scripts/{CONFIG_FILENAME}",
        "reducer_script": "scripts/reduce_m6_remarriage_round2.py",
        "raw_schema": RAW_SCHEMA,
        "raw_stdout_external_filename": (
            "m6_remarriage_round2_selection_full.json"
        ),
        "full_stdout_json": (
            "docs/analysis/m6_remarriage_round2_selection_full.json"
        ),
        ...
        "findings_report": ("docs/analysis/m6_remarriage_round2_selection.md"),
    }
...
def _validate_mean_record(
    value: Any, *, where: str, truth: dict[str, Any]
) -> dict[str, Any]:
    record = _mapping(value, where=where)
...
def _validate_fit_public(
    ...
    *,
    ...
    references = _mapping(
        fit["reference_spells"], where=f"{where}.reference_spells"
    )
    hashes = _mapping(
    ...
    expected_hashes = {
        origin: {
            key: value
            for key, value in references[origin].items()
...
def _validate_aggregate_record(
    ...
    _validate_mean_record(
        aggregate["pooled"], where=f"{where}.pooled", truth=truth["pooled"]
    )
    ...
    for boundary in BOUNDARIES:
        ...
        for law in LAWS:
            ...
            _validate_aggregate_record(
                law_record["mean"], where=f"{law_where}.mean", truth=truth
            )
            _assert_same(
    ...
    for law in LAWS[1:]:
        ...
        for boundary in BOUNDARIES:
            ...
            if event_rows > 0:
                ...
                branch_pass = bool(
                    candidate["widowed_direct_risk_exposure"] > 0.0
