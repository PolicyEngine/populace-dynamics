...
"""Run the prospective round-3 train-only remarriage selector.

This is an analysis helper, not a gate runner.  It implements the unchanged
protocol in ``docs/design/m6_remarriage_learning_plan_round2.md`` with the
prospective implementation amendment in
``docs/design/m6_remarriage_round3_amendment.md``.  It does not import the M6
scorer, read ``gates.yaml`` or ``runs/``, or write files.  Its only machine
output is one strict-JSON document on stdout; progress is flushed to stderr.
...
the repository root, the full run is invoked only from a clean freeze commit::
    ...
    PYTHONDONTWRITEBYTECODE=1 PYTHONWARNINGS='ignore::FutureWarning' \
      ...
      > /safe/path/m6-remarriage-r3-full.json \
      2> /safe/path/m6-remarriage-r3-progress.log
...
SMOKE_SCHEMA = "m6.remarriage.learning_plan.round3.synthetic_smoke.v1"
PREFLIGHT_SCHEMA = "m6.remarriage.learning_plan.round3.preflight.v1"
...
def _load_config() -> dict[str, Any]:
    ...
    if config.get("schema") != "m6.remarriage.round3.selector_config.v1":
        raise ProtocolAbort("unexpected round-3 selector config schema")
    return config
...
def _repository_freeze(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if status:
        raise ProtocolAbort(
            "round-3 selector requires a committed clean freeze; "
...
def _synthetic_event_free_publication_group(
    table: dict[tuple[int, int, str, str], float],
    baseline: dict[tuple[int, int, str, str], float],
) -> dict[str, Any]:
    truth_person_years = pd.DataFrame(
        {
            "person_id": [1, 2, 3],
            "year": [2001, 2001, 2001],
            "marital_state": ["divorced", "divorced", "widowed"],
            "years_since_dissolution": [1, 1, 1],
            "age": [20.0, 40.0, 40.0],
    ...
    truth_events = pd.DataFrame(
        {
            "person_id": [2, 3],
            "year": [2001, 2001],
            "transition": ["remarriage", "remarriage"],
            "origin": ["divorced", "widowed"],
            "years_since_dissolution": [1, 1],
            "age": [40.0, 40.0],
            "weight": [2.0, 3.0],
        }
    )
    ...
    group_records = direct["groups"]
    groups = {record["group"]: record for record in group_records}
    if len(group_records) != 18 or len(groups) != len(group_records):
        raise ProtocolAbort("synthetic publication groups are incomplete")
    group_key = _publication_group_key("divorced", 0, 0)
    record = groups[group_key]
    weight_columns_intact = bool(
        record["risk_rows"] == 1
        and record["event_rows"] == 0
        and record["unmatched_same_year_event_rows"] == 0
        and record["unmatched_same_year_event_weight"] == 0.0
        and record["exposure"] == 1.0
        and record["actual_numerator"] == 0.0
        and record["matchable_numerator"] == 0.0
        and record["actual_rate"] == 0.0
        and record["row_expected_numerator"]
        == table[(0, 0, "divorced", "female")]
    )
    if not weight_columns_intact:
        raise ProtocolAbort(
            "synthetic event-free publication group failed to preserve "
            "weight-bearing columns"
        )
    return {
        "group": group_key,
        "record_path": "_direct_standardization/record/publication_groups",
        "publication_group_count": len(group_records),
        "risk_rows": record["risk_rows"],
        "event_rows": record["event_rows"],
        "exposure": record["exposure"],
        "actual_numerator": record["actual_numerator"],
        "matchable_numerator": record["matchable_numerator"],
        "actual_rate": record["actual_rate"],
        "unmatched_same_year_event_weight": record[
            "unmatched_same_year_event_weight"
        ],
        "weight_bearing_columns_intact": weight_columns_intact,
    }
...
def _synthetic_smoke() -> dict[str, Any]:
    ...
    event_free_publication_group = _synthetic_event_free_publication_group(
        table, baseline
    )
    return {
        "schema": SMOKE_SCHEMA,
        ...
        "checks": {
            ...
            "event_free_publication_group": event_free_publication_group,
            "table_sha256": _table_checksum(table),
        },
    }
