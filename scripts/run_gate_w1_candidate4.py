...
per the candidate-1/2 pattern):
  ...
  assemble   -- read all phase outputs -> runs/gate_w1_candidate4_v1.json
...
ARTIFACT_PATH = ROOT / "runs" / "gate_w1_candidate4_v1.json"
...
def _family_c_pass(fc: dict, gated_fids: list[str]) -> bool:
    """Family C passes iff every GATED fingerprint realises its swap.

    Amendment-3 pair-scope verdict adapter -- the one pre-registered
    build-step exception (issue #42 comment 4964126356). The flip
    (14658d0, ratified cb1ef63) re-scopes the gated C2 check from the
    demoted 4-element ``reversed_to_anchor`` (Kendall tau 1.0 vs the
    anchor) to ``required_swap_realised`` on the contract swap_pair
    ``[payroll_plus_2pp, elimination]`` -- see gates.yaml family_c
    ``fingerprints.c2.gated_check_amendment_3`` and
    ``candidate_procedure.pass_rule``. Only the pair gates; the
    full-ordering statistics (``reversed_to_anchor``,
    ``kendall_tau_vs_required``, ``deployed_order``) are still computed
    by the untouched model and published in family_c and
    verdict.fingerprints (statistics untouched -- verdict read only).
    """
    return bool(
        gated_fids
        and all(
            fc["fingerprints"][fid]["required_swap_realised"]
            for fid in gated_fids
        )
    )
...
def phase_assemble():
    ...
    artifact = {
        "schema_version": "gate_w1_candidate4_v1",
        "run": "gate_w1_candidate4_v1",
