"""Hash-bound one-shot orchestration for the M6 candidate-3 sibling.
...
Amendment 6 changes no family transition.  The family registry therefore
registers candidate 3 as the exact frozen candidate-2 family object, while the
engine registry adds the correlated-refresh operation.  The run-start guard
binds both objects and never mutates the family spec or substitutes candidate
16.
"""
...
SCHEMA_VERSION = "gate_m6_candidate3.v1"
DEFAULT_OUTPUT = Path("runs/gate_m6_candidate3_v1.json")
CANDIDATE1_OUTPUT = Path("runs/gate_m6_candidate1_v1.json")
CANDIDATE2_OUTPUT = Path("runs/gate_m6_candidate2_v1.json")
CANDIDATE1_REGISTRATION_ID = "4990833378"
# Candidate-2 registrations 8-10 are published in their committed incident
# ledgers; registration 11 is published in the scored-verdict commit message
# (ddcd6ef).  This stale-ID fence never opens the candidate-2 run artifact.
CANDIDATE2_REGISTRATION_IDS = frozenset(
    {
        "5015653634",
        "5016328696",
        "5018346030",
...
CONSUMED_CANDIDATE_REGISTRATION_IDS = frozenset(
    {CANDIDATE1_REGISTRATION_ID, *CANDIDATE2_REGISTRATION_IDS}
)
...
CANDIDATE2_FOUR_OF_FIVE_CELLS = ("remarriage.18-64",)
TARGET_SURFACE_EXCLUSIONS = (
...
PREREGISTERED_VALUES: Mapping[str, str] = MappingProxyType(
    {
        "design_commit": ("5b5c9c641d0cf38926d42afb839b45434c4b1b60"),
        "floor_run_sha256": (
        ...
        "selection_evidence.rhostar_rho_grid": (
...
EXPECTED_THREAD_ENVIRONMENT: Mapping[str, str] = MappingProxyType(
    {key: "8" for key in THREAD_ENVIRONMENT_KEYS}
)
EXPECTED_RUNTIME_VERSIONS: Mapping[str, str] = MappingProxyType(
    {
        "python": "3.14.4",
        "numpy": "2.5.1",
        "pandas": "3.0.3",
...
PINNED_POPULACE_ROOT = (
    Path.home() / "m6-sol-lanes" / "populace-pinned"
).resolve()


@dataclass(frozen=True)
class M6Candidate3Identity:
    """The two candidate-3 registry specs and their registry-derived number."""
...
class M6Candidate3DesignedAbort(RuntimeError):
    ...
    def __init__(self, report: Mapping[str, Any]) -> None:
        ...
        message = (
            ...
            else "candidate-3 designed abort"
        )
        super().__init__(str(message))


@dataclass(frozen=True)
class M6Candidate3RunnerOperations:
    """Injectable phase seams for synthetic candidate-3 runner tests."""
...
def _git_text(repository: Path, *arguments: str) -> str:
    try:
        ...
        raise RuntimeError(
            f"candidate-3 runner failed to run `{command}`"
        ) from error
...
def _head_blob(repository: Path, commit_sha: str, path: str) -> bytes:
    try:
        ...
        raise RuntimeError(
            f"candidate-3 runner cannot read the HEAD blob {path!r}"
        ) from error
...
def capture_source_identity(repository: Path) -> M6SourceIdentity:
    ...
    if status:
        raise RuntimeError(
            "candidate-3 run refuses a dirty worktree before any data read"
        )
...
def resolve_candidate3_identity() -> M6Candidate3Identity:
    """Resolve candidate number 3 from both registries, never a default."""
    matches = [
        (int(number), spec)
        for number, spec in engine_candidates.REGISTRY.items()
        if spec is engine_candidates.CANDIDATE_3
    ]
    if len(matches) != 1:
        raise RuntimeError(
            "engine registry must contain exactly one candidate-3 spec"
        )
    number, engine_spec = matches[0]
    if number != 3:
        raise RuntimeError(
            "candidate-3 engine spec is not registered as candidate number 3"
        )
    family_spec = family_candidates.M6_CANDIDATE_3
    if family_spec is not family_candidates.M6_CANDIDATE_2:
        raise RuntimeError(
            "candidate-3 family binding is not the exact frozen candidate-2 "
            "family spec"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "candidate-3 family binding resolved to forbidden candidate 16"
        )
    if not family_spec.candidate_id.startswith("m6_candidate2_"):
        raise RuntimeError(
            "candidate-3 family registry no longer preserves the frozen "
            "candidate-2 canonical identity"
        )
    ...
    if carried(family_spec) != carried(candidate16):
        raise RuntimeError(
            "candidate-3 family sibling changes a non-first-marriage component"
        )
    first_marriage = [
        component
...
def assert_candidate3_identity_is_frozen(
    identity: M6Candidate3Identity,
) -> None:
    """Refuse any unfrozen family or correlated-refresh law before data."""
    candidate2_operations = engine_candidates.CANDIDATE_2.operations
    if (
        identity.engine_spec.operations[: len(candidate2_operations)]
        != candidate2_operations
        or len(identity.engine_spec.operations)
        != len(candidate2_operations) + 1
    ):
        raise RuntimeError(
            "candidate-3 engine does not byte-carry the complete candidate-2 "
            "operation prefix plus one amendment-6 operation"
        )
    correlated = identity.engine_spec.operation(
        engine_candidates.CORRELATED_REFRESH_OPERATION_KIND
    )
    if (
        correlated is None
        or correlated.implementation_id
        != engine_candidates.CORRELATED_REFRESH_OPERATION_ID
        or set(correlated.params) != {"rho"}
        or correlated.params["rho"] != -0.60
    ):
        raise RuntimeError(
            "candidate-3 engine does not bind the frozen rho=-0.60 "
            "correlated-refresh law"
        )
    ...
    if "prefreeze" in identity.family_spec.candidate_id or selected_c is None:
        raise RuntimeError(
            "candidate-3 family registry is not frozen "
    ...
    if isinstance(selected_c, bool) or not isinstance(
        ...
        raise RuntimeError(
            "candidate-3 family selected_c must be a finite positive number"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "candidate-3 family spec does not bind the preregistered "
    ...
    if not isinstance(checksums, Mapping) or not checksums:
        raise RuntimeError(
            "candidate-3 family spec has no registered final-fit checksums"
        )
...
def _editable_source_snapshot(
    ...
    *,
    ...
    """Bind one editable distribution to the frozen lane-local clone."""
    ...
    required_source = source_directory.resolve()
    if source != required_source:
        raise RuntimeError(
            f"{package} editable source is {source}, not {required_source}"
        )
    repository = Path(
        _git_text(source, "rev-parse", "--show-toplevel")
    ).resolve()
    if repository != PINNED_POPULACE_ROOT:
        raise RuntimeError(
            f"{package} repository is not the pinned populace clone"
        )
    status = _git_text(
        repository,
        "status",
    ...
    if head != EXPECTED_POPULACE_HEAD:
        raise RuntimeError(f"{package} editable repository HEAD drifted")
    tree = _git_text(repository, "rev-parse", f"HEAD:{tree_path}")
    if tree != expected_tree:
        raise RuntimeError(f"{package} editable source tree drifted")
    return {
...
def _frozen_gate_environment() -> dict[str, Any]:
    """Fail closed on the lock-addendum runtime and join-barrier conditions."""
    runtime = {
    ...
    if runtime != EXPECTED_RUNTIME_VERSIONS:
        raise RuntimeError(
            f"candidate-3 gate runtime {runtime} differs from "
            f"{dict(EXPECTED_RUNTIME_VERSIONS)}"
        )
    thread_environment = {
    ...
    if wait_policy != "ACTIVE":
        raise RuntimeError(
            "OMP_WAIT_POLICY must equal ACTIVE for the frozen candidate-3 "
            "join-barrier reproduction condition"
        )
    fit_source = _editable_source_snapshot(
        "populace-fit",
        source_directory=(PINNED_POPULACE_ROOT / "packages" / "populace-fit"),
        tree_path="packages/populace-fit/src/populace/fit",
        expected_tree=EXPECTED_POPULACE_FIT_TREE,
    )
    frame_source = _editable_source_snapshot(
        "populace-frame",
        source_directory=(
            PINNED_POPULACE_ROOT / "packages" / "populace-frame"
        ),
        tree_path="packages/populace-frame/src/populace/frame",
        expected_tree=EXPECTED_POPULACE_FRAME_TREE,
    )
    return {
        "runtime": runtime,
...
def _sidecar_snapshot(
    ...
    environment = dict(environment_block())
    environment["candidate3_gate_freeze"] = _frozen_gate_environment()
    contract_ref = asdict(
        ContractRef(
            blob_sha=_git_text(
                repository,
...
def _resolve_pinned_contract(
    ...
    if floor_path.is_absolute() or ".." in floor_path.parts:
        raise ValueError("candidate-3 floor path escapes the repository")
    ...
    if floor_sha != contract.floor_run_sha256:
        raise ValueError(
            "candidate-3 frozen floor sha256 mismatch: "
            f"{floor_sha} != {contract.floor_run_sha256}"
        )
    floor = json.loads(floor_bytes)
    if not isinstance(floor, Mapping):
        raise ValueError("candidate-3 floor artifact must be a mapping")
...
def _assert_selection_semantics(
    ...
    if identity.first_marriage_params.get("selected_c") != selected_c:
        raise RuntimeError(
            "candidate-3 family selected_c differs from its selection ledger"
        )
    if identity.first_marriage_params.get("final_fit_checksums") != checksums:
        raise RuntimeError(
            "candidate-3 family final-fit checksums differ from its ledger"
        )
    if identity.first_marriage_params.get("selection_ledger_sha256") != (
        _sha256_bytes(evidence_blobs["first_marriage_c"])
    ):
        raise RuntimeError(
            "candidate-3 family spec does not bind its exact selection ledger"
        )
    ...
    if registered_q != selected_q:
        raise RuntimeError(
            "candidate-3 engine q differs from its selection ledger"
        )

    rhostar = json.loads(evidence_blobs["rhostar_rho_grid"])
    correlated = identity.engine_spec.operation(
        engine_candidates.CORRELATED_REFRESH_OPERATION_KIND
    )
    registered_rho = (
        None if correlated is None else correlated.params.get("rho")
    )
    selector = rhostar.get("selector", {})
    protocol = rhostar.get("protocol", {})
    cross_pins = rhostar.get("cross_pins", {})
    fences = rhostar.get("fences", {})
    if (
        rhostar.get("schema") != "m6_rhostar_train_only_selection.findings.v1"
        or rhostar.get("status") != "SELECTION_COMPLETE"
        or selector.get("disposition") != "LOCK_ADDENDUM_ELIGIBLE"
    ):
        raise RuntimeError(
            "candidate-3 rho selection ledger is not a completed lock finding"
        )
    if registered_rho != selector.get("selected_rho"):
        raise RuntimeError(
            "candidate-3 engine rho differs from its selection ledger"
        )
    if protocol.get("fixed_q") != registered_q:
        raise RuntimeError(
            "candidate-3 rho ledger fixed q differs from the engine law"
        )
    if (
        cross_pins.get("candidate2_engine_spec_sha256")
        != engine_candidates.CANDIDATE_2.sha256
        or cross_pins.get("qstar_ledger_sha256")
        != _sha256_bytes(evidence_blobs["qstar_q_grid"])
        or cross_pins.get("qstar_selected_q") != registered_q
    ):
        raise RuntimeError("candidate-3 rho-ledger cross-pins drifted")
    if any(
        fences.get(key) is not True
        for key in (
            "no_candidate_1_or_candidate_2_artifact_read",
            "no_gate_score",
            "no_runs_write",
        )
    ):
        raise RuntimeError("candidate-3 rho-ledger information fences drifted")
...
def collect_candidate3_bindings(
    repository: Path,
    *,
    source: M6SourceIdentity,
    identity: M6Candidate3Identity | None = None,
) -> tuple[M6Candidate3Bindings, M6ResolvedContract, bytes]:
    """Collect every compared and record-only binding after the clean check."""
    identity = identity or resolve_candidate3_identity()
...
def _compared_values(bindings: M6Candidate3Bindings) -> dict[str, str]:
    return {
        ...
        "selection_evidence.rhostar_rho_grid": (
            bindings.selection_evidence_sha256s["rhostar_rho_grid"]
        ),
        "selection_evidence.qstar_q_grid": (
...
def assert_preregistered_bindings(bindings: M6Candidate3Bindings) -> None:
    """Abort on drift in exactly the eight preregistered values."""
    observed = _compared_values(bindings)
    if tuple(observed) != PREREGISTERED_COMPARISON_KEYS:
        raise RuntimeError(
            "candidate-3 preregistration comparison scope drifted"
        )
...
def _assert_imported_source_tree(repository: Path) -> None:
    ...
    for observed, required in expected.items():
        if observed != required:
            raise RuntimeError(
                "candidate-3 imported source is outside the guarded tree: "
                f"{observed} != {required}"
            )
...
def _resolve_destination(repository: Path, output: Path | str) -> Path:
    ...
    candidate2 = (repository / CANDIDATE2_OUTPUT).resolve()
    candidate3 = (repository / DEFAULT_OUTPUT).resolve()
    if destination in (candidate1, candidate2):
        refused = (
            CANDIDATE1_OUTPUT
            if destination == candidate1
            else CANDIDATE2_OUTPUT
        )
        raise ValueError(
            f"candidate-3 runner hard-refuses {refused.as_posix()}"
        )
    if destination != candidate3:
        raise ValueError(
            "candidate-3 runner writes exclusively to "
    ...
    for target in (destination, Path(f"{destination}.env.json")):
        if os.path.lexists(target):
            raise FileExistsError(
                f"{target} already exists; candidate 3 is a one-shot run"
            )


def validate_candidate3_registration_id(value: str) -> str:
    """Require a registration unused by either prior scored candidate."""
    registration = m6_runner.validate_registration_id(value)
    numeric = registration.rsplit("-", 1)[-1]
    if numeric in CONSUMED_CANDIDATE_REGISTRATION_IDS:
        raise ValueError(
            "candidate-3 requires a fresh registration and refuses every "
            "consumed candidate-1/candidate-2 registration"
        )
    return registration


def guard_registered_m6_candidate3_run(
    *,
    registration_id: str,
    output: Path | str = DEFAULT_OUTPUT,
    root: Path | str | None = None,
) -> M6Candidate3Preparation:
    ...
    identity = resolve_candidate3_identity()
    assert_candidate3_identity_is_frozen(identity)
    bindings, resolved, sidecar = collect_candidate3_bindings(
        repository,
        source=source,
        identity=identity,
    )
    assert_preregistered_bindings(bindings)
    return M6Candidate3Preparation(
...
def _fit_candidate3(fit_inputs: M6RefitInputs) -> M6RefitBundle:
    identity = resolve_candidate3_identity()
    assert_candidate3_identity_is_frozen(identity)
...
def _run_candidate3_preflight_1(
...
def _fit_postrepair_incumbent_first_marriage(
    ...
    earnings modifier only.  Earnings candidate 3 and every other fitted
    component are the exact objects used by the scored candidate projection.
    """
    if not isinstance(candidate_bundle, M6RefitBundle):
        raise TypeError("candidate-3 comparator requires an M6RefitBundle")
    ...
    if candidate_family is None:
        raise RuntimeError("candidate-3 family fit is absent")
    ...
    if candidate_modifier is None:
        raise RuntimeError("candidate-3 first-marriage modifier is absent")
...
def _preflight_candidate3_first_marriage(
    ...
    if not isinstance(expected, Mapping):
        raise FirstMarriagePreflightAbort(
            "candidate-3 final-fit checksum binding is malformed"
        )
    ...
    if float(audit["c"]) != float(selected_c):
        raise FirstMarriagePreflightAbort(
            "candidate-3 fitted C differs from the registry spec"
        )
    recomputed = dict(recompute_support_aware_first_marriage_checksums(model))
    if recomputed != dict(expected):
        raise FirstMarriagePreflightAbort(
            "candidate-3 final-fit checksums differ from registration"
        )
    return {
        "passed": True,
...
def _write_exclusive(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        raise FileExistsError(
            f"{path} already exists; candidate 3 is a one-shot run"
        ) from None
...
def write_new_candidate3_artifact(
    path: Path,
    ...
    if destination.name in (CANDIDATE1_OUTPUT.name, CANDIDATE2_OUTPUT.name):
        refused = (
            CANDIDATE1_OUTPUT
    ...
    if expected != actual:
        raise ValueError(
            "candidate-3 artifact does not bind its exact environment sidecar"
        )
...
def default_operations() -> M6Candidate3RunnerOperations:
    incumbent = m6_runner.default_operations()
    return M6Candidate3RunnerOperations(
        fit=_fit_candidate3,
        first_marriage_preflight=_preflight_candidate3_first_marriage,
        ...
        preflight_1=_run_candidate3_preflight_1,
        ...
        write=write_new_candidate3_artifact,
    )
...
def _assert_phase_candidate_binding(
    ...
    if not isinstance(specs, Mapping):
        raise RuntimeError("candidate-3 refit did not publish resolved specs")
    ...
    for name in ("family_transitions", "engine_operations"):
        ...
        if specs.get(lineage_name) != expected[name]:
            raise RuntimeError(
                f"candidate-3 refit lineage did not bind {name}"
            )
    engine_id = phase.lineage.get("engine_candidate_id")
    if engine_id != bindings.candidate_spec_ids["engine_operations"]:
        raise RuntimeError("candidate-3 refit lineage has the wrong engine id")
...
def _first_marriage_estimator_delta(
    ...
    Both arms use the landed marital assembler, candidate-3 earnings law, the
        ...
        per_seed.append(
            {
                ...
                "candidate3_minus_postrepair_incumbent": {
    ...
    return {
        ...
        "comparison": (
            ...
            "candidate-3 support-aware estimator"
        ),
        "postrepair_incumbent_family_spec": {
            "candidate_id": family_candidates.CANDIDATE_16.candidate_id,
            "canonical_json_sha256": family_candidates.CANDIDATE_16.sha256,
        },
        "candidate3_disjoint_components_reused_by_object_identity": [
            "household",
            "earnings",
            "disability",
            "claiming_pmfs",
...
def _first_marriage_effect_decomposition(
    ...
    return {
        ...
        "landed_conformance_relabel": {
            "classification": "landed repair; not a candidate-3 model delta",
            "source": {
        ...
        "attribution_limits": {
            ...
            "landed_relabel_is_not_attributed_to_candidate3_estimator": True,
...
def _must_not_regress_artifact(gate_score: Any) -> dict[str, Any]:
    constrained = set(CANDIDATE2_FIVE_OF_FIVE_CELLS) | set(
        CANDIDATE2_FOUR_OF_FIVE_CELLS
    )
    if constrained != set(MUST_NOT_REGRESS_TOLERANCES):
        raise RuntimeError(
            "candidate-3 regression-cell registry differs from program "
            "section 7"
        )
    if constrained & set(TARGET_SURFACE_EXCLUSIONS):
        raise RuntimeError(
            "candidate-3 target surface leaked into the regression block"
        )
    ...
    for seed_score in seeds:
        ...
        if seed in by_seed:
            raise RuntimeError(
                "candidate-3 regression block received a duplicate seed"
            )
        by_seed[seed] = seed_score
    if set(by_seed) != set(MUST_NOT_REGRESS_SEEDS):
        raise RuntimeError(
            "candidate-3 regression seeds differ from the registered 0..4"
        )
    ...
    for seed in MUST_NOT_REGRESS_SEEDS:
        ...
        for cell_score in getattr(seed_score, "cells", ()):
            ...
            if name in cells:
                raise RuntimeError(
                    "candidate-3 regression block received a duplicate cell"
                )
            cells[name] = cell_score
        missing = set(MUST_NOT_REGRESS_TOLERANCES) - set(cells)
        if missing:
            raise RuntimeError(
                "candidate-3 regression block is missing cells "
                f"{sorted(missing)}"
            )
        ...
        per_seed.append(
            {
                "seed": seed,
                "valid": seed_valid,
                "all_constrained_cells_pass": (
                    seed_valid and n_cells_pass == len(records)
                ),
    ...
    for name, tolerance in MUST_NOT_REGRESS_TOLERANCES.items():
        records = {
            int(seed_record["seed"]): seed_record["cells"][name]
            for seed_record in per_seed
        }
        cell_valid = all(record["valid"] for record in records.values())
        n_seed_pass = sum(record["pass"] for record in records.values())
        per_cell[name] = {
            "valid": cell_valid,
            "pass": (
                cell_valid and n_seed_pass >= MUST_NOT_REGRESS_REQUIRED_SEEDS
            ),
            "n_seeds_pass": n_seed_pass,
            "required_seed_passes": MUST_NOT_REGRESS_REQUIRED_SEEDS,
            "seed_pass": {
                str(seed): bool(record["pass"])
                for seed, record in records.items()
            },
            "tolerance": tolerance,
            "provenance": (
                "candidate1_original_threshold"
                if name in ORIGINAL_CANDIDATE1_REGRESSION_CELLS
                else "candidate2_gate_threshold"
            ),
        }
    valid = all(record["valid"] for record in per_cell.values())
    passed = valid and all(record["pass"] for record in per_cell.values())
    return {
        "valid": valid,
        "pass": passed,
        ...
        "tolerance_source": (
            "docs/design/m6_candidate3_program.md section 7; original "
            "candidate-1 thresholds plus candidate-2 gate thresholds"
        ),
        "candidate2_five_of_five_cells": list(CANDIDATE2_FIVE_OF_FIVE_CELLS),
        "candidate2_four_of_five_cells": list(CANDIDATE2_FOUR_OF_FIVE_CELLS),
        "target_surface_exclusions": list(TARGET_SURFACE_EXCLUSIONS),
        "same_seed_conjunction_required": False,
        "per_seed": per_seed,
    }


def _candidate3_acceptance(
...
def assemble_m6_candidate3_artifact(
    *,
    registration_id: str,
    inputs: M6HarnessInputs,
    phase: M6RefitPhase,
    ...
    """Build the candidate-3 payload without a candidate-1 artifact default."""
    ...
    acceptance = _candidate3_acceptance(family_a, must_not_regress)
    conjunction = acceptance["conjunction"]
    run_verdict = _candidate3_run_verdict(
        conjunction,
    ...
    engine_operations = bindings.candidate_specs["engine_operations"][
        "operations"
    ]
    rank_refresh = next(
        operation
        for operation in engine_operations
        if operation["kind"] == engine_candidates.RANK_REFRESH_OPERATION_KIND
    )
    correlated_refresh = next(
        operation
        for operation in engine_operations
        if operation["kind"]
        == engine_candidates.CORRELATED_REFRESH_OPERATION_KIND
    ...
    artifact = {
        ...
        "candidate": {
            ...
            "description": "M6 candidate-3 sibling program",
            ...
            "earnings_rank_refresh_q": rank_refresh["params"]["q"],
            "earnings_refresh_rho": correlated_refresh["params"]["rho"],
        },
        "registration": {
            "issue": 42,
            "registration_id": registration_id,
            "fresh_registration_required": True,
            "one_registered_run": True,
        ...
        "candidate3_acceptance": acceptance,
...
def _revalidate_source_identity(
    preparation: M6Candidate3Preparation,
) -> None:
    observed = capture_source_identity(preparation.repository)
    if observed != preparation.bindings.source:
        raise RuntimeError(
            "candidate-3 source commit changed after the run-start guard"
        )
    _assert_imported_source_tree(preparation.repository)


def _assert_preparation_unchanged(
    expected: M6Candidate3Preparation,
    observed: M6Candidate3Preparation,
    *,
    stage: str,
) -> None:
    if observed != expected:
        raise RuntimeError(
            f"candidate-3 guarded bindings changed {stage}; refusing run"
        )
...
def _designed_abort_report(
    ...
    *,
    ...
    report = {
        "schema_version": "gate_m6_candidate3.designed_abort.v1",
        "status": {
...
def _verified_designed_abort(
    ...
    *,
    ...
    current = guard_registered_m6_candidate3_run(
        registration_id=preparation.registration_id,
    ...
    return M6Candidate3DesignedAbort(
...
def execute_registered_m6_candidate3_run(
    input_plan: M6Candidate3InputPlan,
    *,
    registration_id: str,
    output: Path | str = DEFAULT_OUTPUT,
    root: Path | str | None = None,
    operations: M6Candidate3RunnerOperations | None = None,
    preparation: M6Candidate3Preparation,
) -> dict[str, Any]:
    """Fit/preflight before truth construction, then write the pair last."""
    if not isinstance(input_plan, M6Candidate3InputPlan):
        raise TypeError(
            "candidate-3 execution requires M6Candidate3InputPlan; eager "
            "M6HarnessInputs are prohibited"
        )
    prepared = preparation
    registration = validate_candidate3_registration_id(registration_id)
    if registration != prepared.registration_id:
        raise RuntimeError("candidate-3 registration changed after data load")
    repository = _repository(root)
    if repository != prepared.repository:
        raise RuntimeError("candidate-3 repository changed after data load")
    destination = _resolve_destination(repository, output)
    if destination != prepared.destination:
        raise RuntimeError("candidate-3 output changed after data load")
    current = guard_registered_m6_candidate3_run(
        registration_id=registration,
        output=destination,
        root=repository,
    ...
    artifact = assemble_m6_candidate3_artifact(
    ...
    final = guard_registered_m6_candidate3_run(
        registration_id=registration,
        output=destination,
        root=repository,
    )
...
__all__ = [
    ...
    "CANDIDATE2_REGISTRATION_IDS",
    "CANDIDATE2_FIVE_OF_FIVE_CELLS",
    "CANDIDATE2_FOUR_OF_FIVE_CELLS",
    "CANDIDATE2_OUTPUT",
    "CONFORMANCE_EVIDENCE_PATH",
    "CONFORMANCE_SOURCE_BINDING_KEYS",
    "CONSUMED_CANDIDATE_REGISTRATION_IDS",
    "DEFAULT_OUTPUT",
    "DESIGN_BLOB_PATHS",
    "FIRST_MARRIAGE_GATE_CELL",
    "LANDED_REPAIR_COMMIT",
    "M6Candidate3Bindings",
    "M6Candidate3DesignedAbort",
    "M6Candidate3Identity",
    "M6Candidate3InputPlan",
    "M6Candidate3Preparation",
    "M6Candidate3RunnerOperations",
    "M6SourceIdentity",
    ...
    "TARGET_SURFACE_EXCLUSIONS",
    "assemble_m6_candidate3_artifact",
    "assert_candidate3_identity_is_frozen",
    "assert_preregistered_bindings",
    "capture_source_identity",
    "collect_candidate3_bindings",
    "default_operations",
    "execute_registered_m6_candidate3_run",
    "guard_registered_m6_candidate3_run",
    "resolve_candidate3_identity",
    "validate_candidate3_registration_id",
    "write_new_candidate3_artifact",
]
