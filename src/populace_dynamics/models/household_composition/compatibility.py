"""Build a complete candidate-9 registry compatibility certificate."""
...
def build_compatibility_certificate(
    ...
    *,
    ...
    """Diff every scored candidate-9 quantity and return the certificate."""
    ...
    if (
        ...
        raise RuntimeError("candidate-9 canonical cube axes do not match")
    ...
    if not np.array_equal(canonical_bits, per_seed_bits):
        raise RuntimeError(
            "candidate-9 canonical cube differs from its per-seed rate copies"
        )
    ...
    for block, block_result in reference["per_family_decomposition"].items():
    ...
    return {
        ...
        "protocol": {
            ...
            "outer_rng_note": (
                ...
                "with the committed isolated component tags"
            ),
        },
